...
_CPU_AND_GPU_CODE_ inline void gradientX(DEVICEPTR(Vector4s) *grad, int x, int y, const CONSTPTR(Vector4u) *image, Vector2i imgSize)
{
    ...
    d2.x = image[(x + 1) + (y+1) * imgSize.x].x - image[(x - 1) + (y+1) * imgSize.x].x;
    d2.y = image[(x + 1) + (y+1) * imgSize.x].y - image[(x - 1) + (y+1) * imgSize.x].y;
    d2.z = image[(x + 1) + (y+1) * imgSize.x].z - image[(x - 1) + (y+1) * imgSize.x].z;

    d1.w=d2.w=d3.w=255*2;
