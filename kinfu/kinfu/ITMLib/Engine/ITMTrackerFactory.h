...
namespace ITMLib
{
  namespace Engine
  {
    ...
    class ITMTrackerFactory
    {
      //#################### TYPEDEFS ####################
    private:
      typedef ITMTracker *(*Maker)(const Vector2i&,const ITMLibSettings*,const ITMLowLevelEngine*,ITMScene<TVoxel,TIndex>*);
        ...
        switch(settings->deviceType)
        {
          case ITMLibSettings::DEVICE_CPU:
         {
          ...
          case ITMLibSettings::DEVICE_CUDA:
          {
            ...
            compositeTracker->SetTracker(new ITMWeightedICPTracker_CUDA(
                                             trackedImageSize,
                                             settings->trackingRegime,
                                             settings->noHierarchyLevels,
                                             settings->noICPRunTillLevel,
            ...
            compositeTracker->SetTracker(
              new ITMRenTracker_CUDA<TVoxel, TIndex>(
                            trackedImageSize,
                            settings->trackingRegime,
                            2,
                            lowLevelEngine,
